...
@TeleOp(name = "Blue-LONG-T", group = "Examples")
public class BlueLONG extends OpMode {

    private static final Logger log = LoggerFactory.getLogger(BlueLONG.class);
    private Follower follower;
    private final ElapsedTime parktimer = new ElapsedTime();

    private static final double GOAL_X_Blue =0;
    private static final double GOAL_Y_Blue = 100;

    // Robot starts facing 270° (upfield)
    private final Pose startPose = new Pose(37, 130, Math.toRadians(0));
    ...
    public void loop() {
        ...
        if (gamepad1.cross) {
            ...
            ((DcMotorEx) shooter2).setVelocity(1150 - 500);
            ...
            ((DcMotorEx) shooter2).setVelocity(1160);

            if ((((DcMotorEx) shooter1).getVelocity() >= 1370)
                    && (((DcMotorEx) shooter2).getVelocity() >= 1140)) {
